...
def read_mac_progress(esp, args):
    mac = esp.read_mac()
...
def main(argv=None, esp=None):
    ...
    if operation_args[0] == 'esp':  # operation function takes an ESPLoader connection object
        ...
        if esp.secure_download_mode:
            ...
            mac = read_mac_progress(esp, args)
            print('MAC readout result:')
            print(mac)
            sw.connection_state.emit(f'Connected to {esp.get_chip_description()} [{mac}]')
